module DoubleAgent
  ...
  # Parses and evals BROWSER_DATA into a case statement inside of the _browser_sym method.
  def self.load_browsers!
    ...
    str << 'end'
    module_eval "def self._browser_sym(user_agent); #{str}; end"
  end

  # Parses OS_DATA into OSES, a hash of OSParser objects indexed by their symbol names.
  # Parses and evals OS_DATA into a case statement inside of the _os_sym method.
  def self.load_oses!
    OSES.clear
    str = "case user_agent.to_s\n"
    ...
    str << 'end'
    module_eval "def self._os_sym(user_agent); #{str}; end"
  end
end
