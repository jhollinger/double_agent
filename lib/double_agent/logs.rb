...
module DoubleAgent
  ...
  # parses all matching files into an array of DoubleAgent::LogEntry objects, and returns them.
  ...
  def self.logs(glob_str, options={})
    ...
    parse = if match or ignore
      lambda { |line|
        begin
          entries << LogEntry.new(line) unless (match and line !~ match) or (ignore and line =~ ignore)
    ...
    read = lambda do |f|
      zipped = f =~ /\.gz\Z/i
      return unless defined?(Zlib) or not zipped
